 ...
 * ============================================
 * 同一份渲染逻辑支持两种宿主：
 * 1. my-data.html 弹窗：战绩行点击后打开（与其他模式弹窗交互一致）
 * 2. demolition-match.html 独立页：通过 ?matchId=... 直达（供首页日报代表对局）
 */
(function (global) {
    'use strict';
    ...
    /* ============================================
       渲染器：比赛概览 / 双方计分板 / 数据说明
       ============================================ */
    const BaopoMatchRenderer = {

        renderOverview(root, match, DM) {
            root.innerHTML = '';
        ...
        renderScoreboard(root, match, DM) {
            root.innerHTML = '';
            ...
            root.appendChild(this.buildTeamBlock(match, 'self', '我方队伍'));
            root.appendChild(this.buildTeamBlock(match, 'enemy', '敌方队伍'));

            if (!root.dataset.detailBound) {
        ...
        buildTeamBlock(match, teamKey, label) {
            ...
            players.forEach(p => {
                tbody.appendChild(this.buildPlayerRow(p));
                tbody.appendChild(this.buildPlayerDetailRow(p));
            });
        ...
        buildPlayerDetailRow(p) {
            ...
            hitBox.appendChild(this.buildHitBar('头部', p.headHits, totalHits, 'head'));
            hitBox.appendChild(this.buildHitBar('身体', p.bodyHits, totalHits, 'body'));
            hitBox.appendChild(this.buildHitBar('腿部', p.legHits, totalHits, 'leg'));
            ...
            [
                ...
                ['评级', p.grade]
            ].forEach(([k, v]) => keyBox.appendChild(this.buildKvLine(k, v)));
            ...
            [
                ...
                ['四杀', p.multiKill[4]],
                ['五杀', p.multiKill[5]]
            ].forEach(([k, v]) => multiBox.appendChild(this.buildKvLine(k, String(v))));
            ...
            ].forEach(([k, v]) => ecoBox.appendChild(this.buildKvLine(k, v)));
        ...
        buildHitBar(label, value, total, part) {
    ...
    /* ============================================
       弹窗控制器（my-data.html）
       ============================================ */
    const BaopoMatchModal = {
        open(matchId) {
            const DM = global.DemolitionMock;
            const overlay = document.getElementById('baopo-match-detail-overlay');
            const modal = document.getElementById('baopo-match-detail-modal');
            if (!DM || !overlay || !modal) return;

            const match = DM.getMatchById(matchId);
            if (!match) return;

            this.renderHeader(match, DM);

            const body = document.getElementById('baopo-modal-body');
            if (body) {
                body.innerHTML = '';
                const scoreboard = el('section', 'dm-scoreboard-panel');
                BaopoMatchRenderer.renderScoreboard(scoreboard, match, DM);
        ...
        renderHeader(match, DM) {
            const isVictory = match.result === 'victory';
            const tag = document.getElementById('baopo-modal-result-tag');
            if (tag) {
                tag.classList.toggle('success', isVictory);
                tag.classList.toggle('fail', !isVictory);
            }
            const icon = document.getElementById('baopo-modal-result-icon');
            if (icon) icon.textContent = isVictory ? '✓' : '✗';
            const text = document.getElementById('baopo-modal-result-text');
            if (text) text.textContent = isVictory ? '胜利' : '失败';

            const mapName = document.getElementById('baopo-modal-map-name');
            if (mapName) mapName.textContent = match.mapName + ' · ' + match.queueLabel;

            const mapMeta = document.getElementById('baopo-modal-map-meta');
            if (mapMeta) {
                mapMeta.textContent = match.startedAtLabel + ' | 时长 ' + DM.fmt.duration(match.durationSeconds);
            }

            const score = document.getElementById('baopo-modal-score');
            if (score) {
                score.textContent = match.score.self + ' : ' + match.score.enemy;
                score.classList.toggle('victory', isVictory);
                score.classList.toggle('fail', !isVictory);
            }
        },
        ...
        init() {
            const overlay = document.getElementById('baopo-match-detail-overlay');
            const closeBtn = document.getElementById('baopo-modal-close-btn');
    ...
    /* ============================================
       独立详情页（demolition-match.html）
       ============================================ */
    function initDetailPage() {
        const contentEl = document.getElementById('dm-content');
        if (!contentEl) return; // 非独立详情页宿主
        ...
        BaopoMatchRenderer.renderOverview(document.getElementById('dm-overview'), match, DM);
        BaopoMatchRenderer.renderScoreboard(document.getElementById('dm-scoreboard'), match, DM);
        BaopoMatchRenderer.renderFooter(document.getElementById('dm-footer'), match);
    ...
    document.addEventListener('DOMContentLoaded', () => {
        BaopoMatchModal.init();
        initDetailPage();
    });

    // 导出给战绩列表调用
    global.BaopoMatchModal = BaopoMatchModal;
    global.BaopoMatchRenderer = BaopoMatchRenderer;
