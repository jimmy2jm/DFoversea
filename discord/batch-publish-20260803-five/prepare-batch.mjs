...
const selected = database.builds.filter((build) => build.active && Number(build.publishCount || 0) === 0).slice(0, 5);
if (selected.length !== 5) throw new Error(`Expected five unpublished active builds, found ${selected.length}`);
...
try {
  for (let index = 0; index < selected.length; index += 1) {
    ...
    const post = {
      ...
      thread: {
        name: `[${build.weaponName} · ${build.price}] ${build.buildName}`,
        id: '',
...
await writeFile(path.join(root, 'batch-manifest.json'), `${JSON.stringify({ preparedAt: new Date().toISOString(), refreshedAt: database.refreshedAt, destinationAlias: 'delta-force-official', intervalSeconds: 180, outputs }, null, 2)}\n`, 'utf8');
